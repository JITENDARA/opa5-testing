		...
		onTheMasterPage: {
			...
			actions: {
				...
				iSortTheListOnName: function () {
					return this.iChooseASorter("sortButton", "Sort By <Vbeln>");
				},
				...
				iFilterTheListOnUnitNumber: function () {
					return this.iChooseASorter("filterButton", "<Netwr>", "<100 <Waerk>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("saleSet");
							...
							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].Vbeln;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].Vbeln;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("saleSet", {
								Vbeln: sObjectNotInTheListId
							});
			...
			assertions: {
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("saleSet");
